...
% ============================================================
\section*{S1. Legacy 标签系统与泄漏消融（Phase G 对照）}
\addcontentsline{toc}{section}{S1. Legacy 标签系统与泄漏消融}
% ============================================================

\subsection*{S1.1 Stage 1B 变量角色分类详细定义}
...
\subsection*{S1.5 Stage 1B 验收标准}
\addcontentsline{toc}{subsection}{S1.5 Stage 1B 验收标准}
...
% ============================================================
\section*{S2. Outcome-Anchor 模型细节}
\addcontentsline{toc}{section}{S2. Outcome-Anchor 模型细节}
% ============================================================

\subsection*{S2.1 结局锚点验证模型完整细节}
\addcontentsline{toc}{subsection}{S2.1 结局锚点验证模型完整细节}

结局锚点验证模型（ElasticNet Logistic，solver=SAGA，L1-ratio=0.5）以全量 CPET 变量
（排除 leakage\_guard 标记变量）预测 test\_result 二元结局，采用嵌套5折交叉验证。

\textbf{模型性能}：CV AUC 0.564（±0.016）；Test AUC 0.608；AUPRC 0.203；
Brier Score 0.125；校准截距：待填充；校准斜率：待填充。

\textbf{定位说明}：结局锚点验证器不定义区间，仅提供 outcome\_risk\_tertile 输入置信度引擎，
并验证 final\_zone 的构念效度方向。AUC 处于汇总级 CPET 信息量合理范围（0.55--0.65）内，
本身证实了 final\_zone 与临床结局代理方向一致。

LightGBM 灵敏度分析：CV AUC（待填充），与 ElasticNet 方向一致。

\subsection*{S2.2 Stage 1B 管线输出表结构}
\addcontentsline{toc}{subsection}{S2.2 Stage 1B 管线输出表结构}
...
当前置信度权重（0.40/0.15/0.20/0.25）基于方法学先验。以下替代权重方案用于稳健性检验：

\begin{itemize}
  \item \textbf{等权方案}：四分量等权（0.25/0.25/0.25/0.25）；
    预计 high-confidence 比例偏低（validation 权重降低）
  \item \textbf{完整度优先方案}：completeness 权重提升至 0.60；
    预计 high-confidence 比例升高（字段完整队列）
  \item \textbf{无结局方案}：validation 权重设为 0，其余三分量等比重新分配；
    适用于无 test\_result 的独立外部队列
\end{itemize}

替代权重敏感性分析（待运行）：验收指标（reference Green\%、梯度方向）
在三种替代方案中的一致性结果。

\subsection*{S3.2 Legacy 系统对照（Phase G vs Stage 1B）}
\addcontentsline{toc}{subsection}{S3.2 Legacy 系统对照（Phase G vs Stage 1B）}
...
% ============================================================
\section*{S4. Mahalanobis 异常表型审计}
\addcontentsline{toc}{section}{S4. Mahalanobis 异常表型审计}
% ============================================================

\subsection*{S4.1 Stage 1B 全量输出摘要}
\addcontentsline{toc}{subsection}{S4.1 Stage 1B 全量输出摘要}
...
\subsection*{S4.2 旧 Phase G ML 辅助验证结果（Legacy 保留）}
\addcontentsline{toc}{subsection}{S4.2 旧 Phase G ML 辅助验证结果（Legacy 保留）}
...
\addcontentsline{toc}{subsection}{S4.3 Mahalanobis 异常审计结果}

\textbf{Phase G Mahalanobis 异常区结果}（用于 QC 对照）：
经验切点 P75=3.825，P95=8.788；参考子集 N=969；
$D^2$ vs test\_result 相关 $r$=$-$0.011（方向性弱）。

\textbf{Stage 1B 定位}：Robust Mahalanobis 距离（MinCovDet，$\chi^2_{k,0.975}$ 阈值）
仅用于输入数据的 QC 审计（识别数据录入错误和异常表型样本，优先抽样用于 Stage II），
\textbf{不参与 final\_zone 定义}。anomaly\_flag=True 样本（待填充）应在数据录入核查中优先处理。
...
\subsection*{S5.1 Strict vs Wide Reference Subset 对比}
\addcontentsline{toc}{subsection}{S5.1 Strict vs Wide 对比}

Strict reference subset（N=855）vs Wide reference subset（N=969）稳健性检验：
final\_zone 分布差异（待分层运行后填充）。
...
\begin{itemize}
  ...
  \item 年龄（$<$67岁 / $\geq$67岁）
  \item 方案（踏车 / 平板）
  \item 血压字段完整性（bp 完整 / 缺失）
  \item No-BP 敏感性（排除 bp\_peak\_sys 相关覆盖规则）
\end{itemize}

分层结果（待分层运行后填充）：各分层参考 Green\,>\,50\%、Red\,<\,15\% 一致性。

\subsection*{S5.3 多定义一致性框架（Phase G，Legacy）}
\addcontentsline{toc}{subsection}{S5.3 多定义一致性框架（Phase G，Legacy）}
